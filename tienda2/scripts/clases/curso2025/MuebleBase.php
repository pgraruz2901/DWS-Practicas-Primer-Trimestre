...
abstract class MuebleBase
{
    ...
    function __construct(string $nombre, ?string $fabricante, ?string $pais, ?int $anio, ?string $fechaIniVenta, ?string $fechaFinVenta, ?int $materialPrincipal, ?float $precio)
    {
        ...
        if ($fechaIniVenta !== null) {
            $fechaObj = DateTime::createFromFormat('d/m/Y', $fechaIniVenta);
            if (!$fechaObj) {
                throw new Exception("Fecha de inicio venta inválida: $fechaIniVenta");
            }
            $this->fechaIniVenta = $fechaIniVenta; // se guarda como string
        }

        if ($fechaFinVenta !== null) {
            $fechaObj = DateTime::createFromFormat('d/m/Y', $fechaFinVenta);
            if (!$fechaObj) {
                throw new Exception("Fecha de fin venta inválida: $fechaFinVenta");
            }
    ...
    {
        $string = "MUEBLE de clase " . $this->MaterialPrincipal . " con nombre " . $this->nombre . " , fabricante " . $this->fabricante . " , fabricado en " . $this->pais . " a partir del año " . $this->anio . " , vendido desde " . $this->fechaIniVenta . " hasta " . $this->fechaFinVenta . " , precio " . $this->precio . " de material " . $this->getMaterialDescripcion();
        $caracteristicas = $this->exportarCaracteristicas();
        if (!empty(trim($caracteristicas))) {
    ...
    public function añadir(...$args)
    {
        $total = count($args);
        if ($total % 2 !== 0) {
            array_pop($args); // quitar el último si no hay par
            $total--;
        }
